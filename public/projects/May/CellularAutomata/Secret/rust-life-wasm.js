...
(function (root, factory) {
    if (typeof define === "function" && define.amd) {
        ...
        Rust.rust_life_wasm = factory();
    }
}(this, async function () {
    return (async function (module_factory) {
        var instance = module_factory();

        if (typeof process === "object" && typeof process.versions === "object" && typeof process.versions.node === "string") {
            var fs = require("fs");
            var path = require("path");
            var wasm_path = path.join(__dirname, "rust-life-wasm.wasm");
            var buffer = fs.readFileSync(wasm_path);
            var mod = new WebAssembly.Module(buffer);
            var wasm_instance = new WebAssembly.Instance(mod, instance.imports);
            return instance.initialize(wasm_instance);
        } else {
            var file = await fetch("rust-life-wasm.wasm", { credentials: "same-origin" });
            let bytes = await file.arrayBuffer();
            let mod = await WebAssembly.compile(bytes);
            let wasm_instance = WebAssembly.instantiate(mod, instance.imports);

            return wasm_instance
                .then(function (wasm_instance) {
                    var exports = instance.initialize(wasm_instance);
                    console.log("Finished loading Rust wasm module 'rust_life_wasm'");
                    return exports;
                })
                .catch(function (error) {
                    console.log("Error loading Rust wasm module 'rust_life_wasm':", error);
                    throw error;
                });
        }
    }(function () {
        ...
        return {
            imports: {
                env: {
                    ...
                    "__cargo_web_snippet_91f5bc262bc731b1bcb0ea335cdc54d9b635b1e7": function ($0) {
